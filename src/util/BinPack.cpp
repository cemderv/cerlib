...
BinPack::BinPack()
    : m_bin_width(0)
    , m_bin_height(0)
    , m_new_free_rectangles_last_size(0)
{
}
...
auto BinPack::insert(int32_t width, int32_t height) -> std::optional<Rect>
{
...
auto BinPack::insert(std::vector<Size>& rects, std::vector<Rect>& dst) -> void
{
    dst.clear();
...
auto BinPack::place_rect(const Rect& node) -> void
{
...
auto BinPack::score_rect(int  width,
                         int  height,
                         int& score1,
                         int& score2) const -> std::optional<Rect>
{
...
auto BinPack::occupancy() const -> double
{
    ...
    return static_cast<double>(usedSurfaceArea) /
                                         ...
                                         int& best_short_side_fit) const -> std::optional<Rect>
{
    ...
    {
        const int areaFit = free_rectangle.width * free_rectangle.height - width * height;
        ...
        if (free_rectangle.width >= width && free_rectangle.height >= height)
        {
            ...
            if (areaFit < best_area_fit ||
                (areaFit == best_area_fit && short_side_fit < best_short_side_fit))
            {
...
auto BinPack::insert_new_free_rectangle(const Rect& newFreeRect) -> void
{
    ...
    for (size_t i = 0; i < m_new_free_rectangles_last_size;)
    {
        ...
        if (is_contained_in(newFreeRect, m_new_free_rectangles[i]))
        {
            return;
        }

        // Does this new free rect obsolete a previous new free rect?
        if (is_contained_in(m_new_free_rectangles[i], newFreeRect))
        {
    ...
    m_new_free_rectangles.push_back(newFreeRect);
}

auto BinPack::prune_free_list() -> void
{
    // Test all newly introduced free rectangles against old free rectangles.
    for (size_t i = 0; i < m_free_rectangles.size(); ++i)
        for (size_t j = 0; j < m_new_free_rectangles.size();)
        {
            if (is_contained_in(m_new_free_rectangles[j], m_free_rectangles[i]))
            {
            ...
            else
            {
                ...
                // in size)
                assert(!is_contained_in(m_free_rectangles[i], m_new_free_rectangles[j]));
                ++j;
            }
        }
