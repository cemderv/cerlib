...
static const void* misc_image_data_upload(
    ...
    uint32_t                                            mipmap)
{
    //  return uploader.image->faces.at(arrayIndex).mipmaps.at(mipmap).data.data();
                                            ...
                                            bool                       generate_mipmaps)
{
    ...
    if (generate_mipmaps && !is_hdr)
    {
        ...
        for (uint32_t i = 0; i < mipmap_count - 1; ++i)
        {
            ...
            std::unique_ptr<std::byte[]> dstData =
                std::make_unique<std::byte[]>(output_width * output_height * 4);

            stbir_resize_uint8_srgb(static_cast<const stbi_uc*>(src_data),
                                    static_cast<int>(w),
                                    static_cast<int>(h),
                                    0,
                                    reinterpret_cast<stbi_uc*>(dstData.get()),
                                    static_cast<int>(output_width),
                                    static_cast<int>(output_height),
                                    0,
                                    STBIR_RGBA);

            mipmap_datas.push_back(std::move(dstData));
