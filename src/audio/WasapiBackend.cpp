...
{
  public:
    MMNotificationClient(WASAPIData* aData)
...
static void wasapiThread(LPVOID aParam)
{
    ...
    while (WAIT_OBJECT_0 != WaitForSingleObject(data->audioProcessingDoneEvent, 0))
    {
        ...
        else
        {
            ...
            bool reinitFailed = FAILED(
                data->deviceEnumerator->GetDefaultAudioEndpoint(eRender, eConsole, &data->device));
            if (!reinitFailed)
            {
                reinitFailed =
                    FAILED(data->device->Activate(__uuidof(IAudioClient),
                                                  CLSCTX_ALL,
                                                  0,
            ...
            {
                reinitFailed =
                    FAILED(data->audioClient->Initialize(AUDCLNT_SHAREMODE_SHARED,
            ...
            {
                reinitFailed = FAILED(
                    data->audioClient->GetService(__uuidof(IAudioRenderClient),
...
result wasapi_init(AudioDevice* engine,
                   size_t       aFlags,
                   size_t /*aSamplerate*/,
                   size_t aBuffer,
                   size_t /*aChannels*/)
{
    ...
    /*HRESULT result = */ data->deviceEnumerator->RegisterEndpointNotificationCallback(
        data->notificationClient);
    if (FAILED(data->deviceEnumerator->GetDefaultAudioEndpoint(eRender, eConsole, &data->device)))
    {
        return UNKNOWN_ERROR;
    }
    if (FAILED(data->device->Activate(__uuidof(IAudioClient),
                                      CLSCTX_ALL,
                                      0,
                                      reinterpret_cast<void**>(&data->audioClient))))
    {
