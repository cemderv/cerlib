            ...
            if (mouse_button_clicked != -1 && g.ActiveId != id)
            {
                             ...
                             ImGuiButtonFlags_PressedOnClickReleaseAnywhere))
                {
                    ...
                    else
                    {
                        FocusWindow(
                            window,
                            ImGuiFocusRequestFlags_RestoreFocusedChild); // Still need to focus and
                                                                         // bring to front, but try
                                                                         // to avoid losing NavId
                        ...
                        ImDrawFlags   flags)
{
    ...
    if (held && allow_interaction && grab_h_norm < 1.0f)
    {
        ...
        if (g.ActiveIdIsJustActivated)
        {
            // On initial click when held_dir == 0 (clicked over grab): calculate the distance
            // between mouse and the center of the grab
                  ...
                  const ImVec4& border_col)
{
    ...
    if (border_size > 0.0f)
        window->DrawList
            ->AddRect(bb.Min, bb.Max, GetColorU32(border_col), 0.0f, ImDrawFlags_None, border_size);
                        ...
                        const ImVec4& tint_col)
{
    ...
    return ImageButtonEx(window->GetID(str_id),
                         user_texture_id,
                         image_size,
                         uv0,
                         uv1,
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    if (!is_indeterminate || overlay != NULL)
    {
        ...
        if (overlay_size.x > 0.0f)
        {
            ...
            RenderTextClipped(
                ImVec2(
                    ImClamp(text_x, bb.Min.x, bb.Max.x - overlay_size.x - style.ItemInnerSpacing.x),
                    bb.Min.y),
                bb.Max,
                overlay,
                NULL,
...
bool ImGui::TextLink(const char* label)
{
    ...
    window->DrawList->AddLine(ImVec2(bb.Min.x, line_y),
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness)
{
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        // Before Tables API happened, we relied on Separator() to span all columns of a Columns()
        // set. We currently don't need to provide the same feature for tables because tables
        // naturally have border features.
...
void ImGui::Separator()
{
    ...
    // FIXME: We cannot g.Style.SeparatorTextBorderSize for thickness as it relates to
    // SeparatorText() which is a decorated separator, not defaulting to 1.0f.
...
void ImGui::SeparatorText(const char* label)
{
    ...
    // highlight)
    // - this high-level entry point to allow formatting? (which in turns may require ID separate
    ...
    if (g.ActiveId == id)
    {
                 ...
                 g.ActiveIdSource == ImGuiInputSource_Gamepad)
        {
            ...
            if (input_delta != 0.0f)
            {
                ...
                {
                    if ((v_range_f >= -100.0f && v_range_f <= 100.0f && v_range_f != 0.0f) ||
                        tweak_slow)
                        ...
                        ImGuiInputTextFlags flags)
{
    ...
    flags |=
        ...
        (ImGuiInputTextFlags)
            ImGuiInputTextFlags_NoMarkEdited; // We call MarkItemEdited() ourselves by comparing the
                                              // actual data rather than the string.
                                    ...
                                    bool          stop_on_new_line)
{
    ...
    while (s < text_end)
    {
        ...
        const float char_width = ((int)c < font->IndexAdvanceX.Size ? font->IndexAdvanceX.Data[c]
...
static float STB_TEXTEDIT_GETWIDTH(ImGuiInputTextState* obj, int line_start_idx, int char_idx)
{
    ...
    ImTextCharFromUtf8(&c,
                       obj->TextA.Data + line_start_idx + char_idx,
...
static void STB_TEXTEDIT_LAYOUTROW(StbTexteditRow* r, ImGuiInputTextState* obj, int line_start_idx)
{
    ...
    const ImVec2 size           = InputTextCalcTextSize(obj->Ctx,
                                              text + line_start_idx,
                                              text + obj->CurLenA,
                                              &text_remaining,
                                              NULL,
                        ...
                        void*                  callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        // Prevent NavActivation from Tabbing when our widget accepts Tab inputs: this allows
    ...
    if ((init_state && g.ActiveId != id) || init_changed_specs || init_reload_from_user_buf)
    {
        ...
        // From the moment we focused we are normally ignoring the content of 'buf' (unless we are
        // in read-only mode)
    ...
    if (g.ActiveId == id)
    {
        ...
        // Edit in progress
        const float mouse_x =
            (io.MousePos.x - frame_bb.Min.x - style.FramePadding.x) + state->Scroll.x;
        const float mouse_y =
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        // FIXME-OSX: Missing support for Alt(option)+Right/Left = go to end of line, or next line
        // if already in end of line.
        ...
        if (apply_edit_back_to_user_buffer)
        {
                  ...
                  ImGuiInputTextFlags_CallbackEdit | ImGuiInputTextFlags_CallbackAlways)) != 0)
            {
                ...
                if (event_flag)
                {
                    ...
                    // FIXME-OPT: Undo stack reconcile needs a backup of the data until we rework
                    // API, see #7925
    ...
    if (render_cursor || render_selection)
    {
...

        {
            ...
            const char* selmin_ptr = render_selection ? text_begin + ImMin(state->Stb->select_start,
                                                                           state->Stb->select_end)
                                                      : NULL;
            ...
            cursor_offset.x =
                InputTextCalcTextSize(&g, ImStrbol(cursor_ptr, text_begin), cursor_ptr).x;
        ...
        // FIXME-OPT: Multiline could submit a smaller amount of contents to AddText() since we
        // already iterated through it.
...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState* state)
{
                   ...
                   ImGuiChildFlags_Borders | ImGuiChildFlags_ResizeY)) // Visualize undo state
    {
        ...
        for (int n = 0; n < IMSTB_TEXTEDIT_UNDOSTATECOUNT; n++)
        {
            ...
            if (undo_rec_type == ' ')
                BeginDisabled();
            const int   buf_preview_len = (undo_rec_type != ' ' && undo_rec->char_storage != -1)
...
bool ImGui::TreeNodeUpdateNextOpen(ImGuiID storage_id, ImGuiTreeNodeFlags flags)
{
    ...
    {
        is_open =
            storage->GetInt(storage_id, (flags & ImGuiTreeNodeFlags_DefaultOpen) ? 1 : 0) != 0;
    }
                       ...
                       const ImVec2&        size_arg)
{
    ...
    if (!is_visible)
        if (!is_multi_select || !g.BoxSelectState.UnclipMode ||
            !g.BoxSelectState.UnclipRect.Overlaps(
                bb)) // Extra layer of "no logic clip" for box-select support (would be more
                     // overhead to add to ItemAdd)
            return false;
    ...
    else
    {
        ...
        // - This is not exposed as it won't nicely work with some user side handling of
        // shift/control
        // - We cannot do 'if (g.NavJustMovedToId != id) { selected = false; pressed = was_selected;
        // }' for two reasons
        //   - (1) it would require focus scope to be set, need exposing PushFocusScope() or
        //   equivalent (e.g. BeginSelection() calling PushFocusScope())
...
ImGuiTypingSelectRequest* ImGui::GetTypingSelectRequest(ImGuiTypingSelectFlags flags)
{
    ...
    // - SingleCharMode is always set for first input character, because it usually leads to a
    // "next".
    if (flags & ImGuiTypingSelectFlags_AllowSingleCharMode)
    {
        ...
        data->SingleCharModeLock |=
            (single_char_count >=
             TYPING_SELECT_SINGLE_CHAR_COUNT_FOR_LOCK); // From now on we stop search matching to
                           ...
                           ImGuiMultiSelectFlags ms_flags)
{
    ...
    // GetForegroundDrawList()->AddRect(bs->BoxSelectRectPrev.Min, bs->BoxSelectRectPrev.Max,
    // IM_COL32(255,0,0,200), 0.0f, 0, 3.0f);
...
static void DebugLogMultiSelectRequests(const char* function, const ImGuiMultiSelectIO* io)
{
    ...
    for (const ImGuiSelectionRequest& req : io->Requests)
    {
        ...
        if (req.Type == ImGuiSelectionRequestType_SetRange)
            IMGUI_DEBUG_LOG_SELECTION("[selection] %s: Request: SetRange %" IM_PRId64
                                      "..%" IM_PRId64 " (0x%" IM_PRIX64 "..0x%" IM_PRIX64
                                      ") = %d (dir %d)\n",
                                      function,
                                      req.RangeFirstItem,
                                      req.RangeLastItem,
...
static ImRect CalcScopeRect(ImGuiMultiSelectTempData* ms, ImGuiWindow* window)
{
    ...
    {
        // When a table, pull HostClipRect, which allows us to predict ClipRect before first
        // row/layout is performed. (#7970)
        ...
        scope_rect.Min =
            ImMin(scope_rect.Min + ImVec2(window->DecoInnerSizeX1, window->DecoInnerSizeY1),
                  scope_rect.Max);
        return scope_rect;
    }
}
                                            ...
                                            int                   items_count)
{
    ...
    ms->KeyMods = g.NavJustMovedToId ? (g.NavJustMovedToIsTabbing ? 0 : g.NavJustMovedToKeyMods)
                                     : g.IO.KeyMods;
    ...
    if (ms->IsFocused)
    {
        ...
        // - Only claim shortcut if selection is not empty, allowing further presses on Escape to
        // e.g. leave current child window.
...
ImGuiMultiSelectIO* ImGui::EndMultiSelect()
{
    ...
    if (ms->IsFocused)
    {
        // We currently don't allow user code to modify RangeSrcItem by writing to BeginIO's
        // version, but that would be an easy change here.
        if (ms->IO.RangeSrcReset ||
            (ms->RangeSrcPassedBy == false &&
             ms->IO.RangeSrcItem !=
                 ImGuiSelectionUserData_Invalid)) // Can't read storage->RangeSrcItem here -> we
                                                  // want the state at begining of the scope (see
                ...
                if (ms->Flags & ImGuiMultiSelectFlags_ScopeRect)
                    SetNavID(0,
                             ...
                             ImRect(g.IO.MousePos,
                                    g.IO.MousePos)); // Automatically switch FocusScope for initial
                                                     // click from void to box-select.
            }
        }
...
void ImGui::SetNextItemSelectionUserData(ImGuiSelectionUserData selection_user_data)
{
    ...
    // This designed so widgets can also cheaply set this before calling ItemAdd(), so we are not
    // tied to MultiSelect api.
    ImGuiContext& g                  = *GImGui;
...
void ImGui::MultiSelectItemHeader(ImGuiID id, bool* p_selected, ImGuiButtonFlags* p_button_flags)
{
    ...
    if (ms->IsFocused)
    {
        ...
        // When using SHIFT+Nav: because it can incur scrolling we cannot afford a frame of lag with
        // the selection highlight (otherwise scrolling would happen before selection) For this to
        // work, we need someone to set 'RangeSrcPassedBy = true' at some point (either clipper
        // either SetNextItemSelectionUserData() function)
        if (ms->IsKeyboardSetRange)
        {
            ...
            if (is_range_dst && storage->RangeSrcItem ==
...
void ImGui::MultiSelectItemFooter(ImGuiID id, bool* p_selected, bool* p_pressed)
{
    ...
    if (ms->BoxSelectId != 0)
        if (ImGuiBoxSelectState* bs = GetBoxSelectState(ms->BoxSelectId))
        {
                ...
                (rect_overlap_prev && !rect_overlap_curr))
            {
                ...
                {
                    pressed =
                        true; // First item act as a pressed: code below will emit selection request
    ...
    // Unlike Space, Enter doesn't alter selection (but can still return a press) unless current
    // item is not selected. The later, "unless current item is not select", may become optional? It
    // seems like a better default if Enter doesn't necessarily open something (unlike e.g. Windows
    ...
    if (pressed && (!enter_pressed || !selected))
    {
        ...
        if ((flags & ImGuiMultiSelectFlags_NoAutoClear) == 0)
        {
            ...
            if (is_singleselect)
                ...
                request_clear =
                    (flags & ImGuiMultiSelectFlags_NoAutoClearOnReselect) ? !selected : true;
            else if ((input_source == ImGuiInputSource_Keyboard ||
                      input_source == ImGuiInputSource_Gamepad) &&
                     is_shift && !is_ctrl)
                request_clear = true; // With is_shift==false the RequestClear was done in BeginIO,
        ...
        if (is_shift && !is_singleselect)
        {
            ...
            else
            {
                ...
                if (ms->IsKeyboardSetRange)
                    range_selected =
                        (storage->RangeSelected != -1) ? (storage->RangeSelected != 0) : true;
                else
                                                            ...
                                                            int selection_order)
{
    ...
    if (selected && !is_contained)
        storage->Data.push_back(
            ImGuiStoragePair(id, selection_order)); // Push unsorted at end of vector, will be
    ...
    for (ImGuiSelectionRequest& req : ms_io->Requests)
    {
        if (req.Type == ImGuiSelectionRequestType_SetAll)
        {
            ...
            if (req.Selected)
            {
                ...
                for (int idx = 0; idx < ms_io->ItemsCount; idx++, _SelectionOrder++)
                    ImGuiSelectionBasicStorage_BatchSetItemSelected(this,
        ...
        else if (req.Type == ImGuiSelectionRequestType_SetRange)
        {
            ...
            if (selection_changes == 1 || (selection_changes < Size / 100))
            {
                ...
                // Technically we could do a single copy with a little more work (sort sequential
                // SetRange requests)
            ...
            else
            {
                ...
                // Use req.RangeDirection to set order field so that shift+clicking from 1 to 5 is
                // different than shift+clicking from 5 to 1
                const int size_before_amends = _Storage.Data.Size;
                int       selection_order =
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
                                  ...
                                  // (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (g.HoveredWindow == window && child_menu_window != NULL)
        {
            ...
            const float pad_farmost_h = ImClamp(ImFabs(ta.x - tb.x) * 0.30f,
                                                ref_unit * 0.5f,
    ...
    {
        // Don't reopen/recycle same menu level in the same frame if it is a different menu ID,
        // first close the other menu and yield for a frame.
        OpenPopup(label);
    }
    else if (want_open)
    {
    ...
    if (menu_is_open)
    {
        ...
        if (menu_is_open)
        {
            ...
            // Perform an init request in the case the popup was already open (via a previous mouse
            // hover)
...
bool ImGui::MenuItemEx(
    const char* label, const char* icon, const char* shortcut, bool selected, bool enabled)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
    ...
    // Draw separator
    // (it would be misleading to draw this in EndTabBar() suggesting that it may be drawn over
    // tabs, as tab bar are appendable)
    const ImU32 col = GetColorU32(
        (flags & ImGuiTabBarFlags_IsFocused) ? ImGuiCol_TabSelected : ImGuiCol_TabDimmedSelected);
