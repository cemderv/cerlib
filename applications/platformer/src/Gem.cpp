...
Gem::Gem(Level* level, Vector2 position, bool is_super_gem)
    : m_level(level)
    , m_base_position(position)
    , m_is_super_gem(is_super_gem)
{
    m_texture = Image{"sprites/gem.png"};
    m_origin  = m_texture.size() / 2;
    m_collected_sound =
        is_super_gem ? Sound{"sounds/super_gem_collected.wav"} : Sound{"sounds/gem_collected.wav"};
}

void Gem::update(GameTime time)
{
    ...
    m_bounce = sin(t) * bounce_height * m_texture.heightf();
}
...
void Gem::draw() const
{
    draw_sprite({
        ...
        .color    = m_is_super_gem ? cornflowerblue * 2.0f : yellow,
        .origin   = m_origin,
    });
}

Vector2 Gem::position() const
{
    return m_base_position + Vector2{0, m_bounce};
}

Circle Gem::bounding_circle() const
{
