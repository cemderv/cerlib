...
TEST_CASE("Vector2", "[math]")
{
    ...
    {
        REQUIRE(cer::are_equal_within({}, {}));
        REQUIRE(cer::are_equal_within({0.1f, 0.2f}, {0.1f, 0.2f}));
        REQUIRE(cer::are_equal_within({0.2f, 0.1f}, {0.2f, 0.1f}));
        REQUIRE_FALSE(cer::are_equal_within({0.1f, 0.2f}, {0.2f, 0.1f}));
    ...
    SECTION("normalized")
    {
        ...
        REQUIRE(are_equal_within(normalize(Vector2{1, 2}), Vector2{0.4472136f, 0.8944272f}));
    }

    SECTION("dot")
    {
        REQUIRE(cer::is_zero(dot(Vector2{}, {})));
        REQUIRE(cer::is_zero(dot(Vector2{}, {1, 0})));
        REQUIRE(cer::is_zero(dot(Vector2{}, {0, 1})));
        REQUIRE(cer::is_zero(dot(Vector2{}, {0.5f, 0.5f})));
    ...
    SECTION("min")
    {
        REQUIRE(cer::min({}, {}) == Vector2{});
        REQUIRE(cer::min({}, {1, 2}) == Vector2{});
        REQUIRE(cer::min({1, 2}, {1.5f, 2}) == Vector2{1, 2});
        REQUIRE(cer::min({1, 2}, {1.5f, 1.25f}) == Vector2{1, 1.25f});
        REQUIRE(cer::min({-0.5f, 0.5f}, {0.5f, -2}) == Vector2{-0.5f, -2});
    }

    SECTION("max")
    {
        REQUIRE(cer::max({}, {}) == Vector2{});
        REQUIRE(cer::max({}, {1, 2}) == Vector2{1, 2});
        REQUIRE(cer::max({1, 2}, {1.5f, 2}) == Vector2{1.5f, 2});
        REQUIRE(cer::max({1, 2}, {1.5f, 1.25f}) == Vector2{1.5f, 2});
        REQUIRE(cer::max({-0.5f, 0.5f}, {0.5f, -2}) == Vector2{0.5f, 0.5f});
    }
