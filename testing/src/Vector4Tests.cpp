...
TEST_CASE("Vector4", "[math]")
{
    ...
    SECTION("dot")
    {
        REQUIRE(is_zero(dot(Vector4(), Vector4())));
        REQUIRE(dot(Vector4(), {1, 0, 0, 0}) == 0.0f);
        REQUIRE(dot(Vector4(), {0, 1, 0, 0}) == 0.0f);
        REQUIRE(dot(Vector4(), {0, 0, 1, 0}) == 0.0f);
        REQUIRE(dot(Vector4(), {0, 0, 0, 1}) == 0.0f);
        REQUIRE(dot(Vector4(), {0.5f, 0.5f, 0.5f, 0.5f}) == 0.0f);
        REQUIRE(dot(Vector4(0.5f, 0.5f, 0.5f, 0.5f), {1, 0, 0, 0}) == 0.5f);
        REQUIRE(dot(Vector4(0.5f, 0.5f, 0.5f, 0.5f), {0, 1, 0, 0}) == 0.5f);
    }

    SECTION("length")
    {
        REQUIRE(is_zero(length(Vector4())));
        REQUIRE(length(Vector4(1, 0, 0, 0)) == 1.0f);
        REQUIRE(length(Vector4(0, 1, 0, 0)) == 1.0f);
        REQUIRE(length(Vector4(1, 1, 0, 0)) == std::sqrt(2.0f));
        REQUIRE(length(Vector4(0.45f, 0, 0, 0)) == 0.45f);
    }

    SECTION("length_squared")
    {
        REQUIRE(is_zero(length_squared(Vector4())));
    ...
    SECTION("min")
    {
        REQUIRE(min(Vector4(), Vector4()) == Vector4());
        REQUIRE(min(Vector4(), {1, 2, 3, 4}) == Vector4());
        REQUIRE(min({1, 2, 3, 4}, {1.5f, 2, 3, 4}) == Vector4(1, 2, 3, 4));
        REQUIRE(min({1, 2, 3, 4}, {1.5f, 1.25f, 1.75f, 2}) == Vector4(1, 1.25f, 1.75f, 2));
        REQUIRE(min({-0.5f, 0.5f, -0.5f, 0.2f}, {0.5f, -3.5f, 0, 0.3f}) ==
                Vector4(-0.5f, -3.5f, -0.5f, 0.2f));
    }

    SECTION("max")
    {
        REQUIRE(max(Vector4(), Vector4()) == Vector4());
        REQUIRE(max(Vector4(), {1, 2, 3, 4}) == Vector4(1, 2, 3, 4));
        REQUIRE(max({1, 2, 3, 4}, {1.5f, 2, 3, 4}) == Vector4(1.5f, 2, 3, 4));
        REQUIRE(max({1, 2, 3, 4}, {1.5f, 1.25f, 1.75f, 2}) == Vector4(1.5f, 2, 3, 4));
        REQUIRE(max({-0.5f, 0.5f, -0.5f, 0.2f}, {0.5f, -3.5f, 0, 0.3f}) ==
